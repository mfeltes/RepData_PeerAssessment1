...
```
The mean and median daily step totals are `r mean1` and `r median1` steps respectively.
...
```
Maximal average step count is `r max$intstep` and is observed at `r max$interval` minutes

## Imputing missing values
```{r}
missing <- sum(!complete.cases(all.data))
```
**all.data** contains many missing values... in fact, `r missing` rows are incomplete!
...
```
The mean and median daily step totals are `r mean2` and `r median2` steps respectively.
