...
```{r Fig1-HistogramDailyStep, warning=FALSE}
...
```
The mean and median daily step totals are <span style="color: red;">`r mean1`</span> and <span style="color: red;">`r median1`</span> steps respectively.
...
```
Maximal average step count is <span style="color: red;">`r max$intstep`</span> and is observed at <span style="color: red;">`r max$interval`</span> minutes

## Imputing missing values
```{r}
missing <- sum(!complete.cases(all.data))
```
**all.data** contains many missing values... in fact, <span style="color: red;">`r missing`</span> rows are incomplete!
...
```{r Fig3-HistogramDailyStepClean}
...
```
The mean and median daily step totals are <span style="color: red;">`r mean2`</span> and <span style="color: red;">`r median2`</span> steps respectively.
...
```{r Fig4-WeekdayWeekendActivityPatterns}
